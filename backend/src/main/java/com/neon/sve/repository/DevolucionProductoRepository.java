...
public interface DevolucionProductoRepository extends JpaRepository<DevolucionProducto, Long> {
  ...
  @Query("""
          ...
          WHERE d.id_detalle_ingreso.id = :idDetalle
          AND d.reposicionAplicada = false
          AND d.id_serie_producto IS NULL
      """)
  boolean existsManipuladaSinSerie(@Param("idDetalle") Long idDetalle);
