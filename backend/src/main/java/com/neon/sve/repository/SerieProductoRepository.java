...
public interface SerieProductoRepository extends JpaRepository<SerieProducto, Long> {
        ...
        @Query("SELECT s FROM SerieProducto s WHERE s.id_detalle_ingreso.id = :idDetalleIngreso")
        List<SerieProducto> buscarPorIdDetalleIngreso(@Param("idDetalleIngreso") Long idDetalleIngreso);
